<template>
  <div class="createPost-container">
    <div>
      ...
      <!--      <VodPlayer id="videoElement"></VodPlayer>-->
      <video-player ref="livePlayer" :options="videoOptions"/>
    </div>
...
import VideoPlayer from '@/components/Player/LivePlayer'
import { getLive, resetKey } from '@/api/live/live'

export default {
  name: 'BlogDetail',
  components: { Sticky, mavonEditor, ImagePicker, VodPlayer, VideoPlayer },
  props: {
    live: {},
    isEdit: {
      type: Boolean,
  ...
  data() {
    return {
      videoOptions: {
        ...
        sources: [
          {
            //src: 'http://192.168.86.16:8080/live/4a56d0f3-4556-4781-81de-95e982ef79b0.m3u8',
            src: '',
  ...
  methods: {
    ...
    getLive(id) {
      getLive(id).then(response => {
        this.msgSuccess('直播加载成功')
        this.live = response.data
        this.videoOptions.sources[0].src = this.live.url + '/' + this.live.key + '.m3u8'
        console.log(this.videoOptions.sources.src)
        this.$nextTick(() => this.$refs.livePlayer.init())
